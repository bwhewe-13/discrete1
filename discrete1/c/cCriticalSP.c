...
// void vacuum(void *flux, void *half, void *external, void *v_total, void *area_one, 
//             void *area_two, void *dir, void *wgt){
void vacuum(void *flux, void *half, void *external, void *v_total, void *dir, void *wgt, double delta){
    ...
    for(int n=0; n < N; n++){
        ...
        if (mu[n] < 0){
            ...
            for(unsigned ii = (LENGTH); ii-- > 0; ){
                ...
                //           (alpha_plus) + total[ii]);
                psi = (-1 * mu[n] * (SA[ii+1] + SA[ii]) * psi_ihalf + 1/w[n] * (SA[ii+1] - SA[ii])
                      * (alpha_plus + alpha_minus) * psi_nhalf[ii] + source[ii]) /
                    (-2 * mu[n] * SA[ii] + 2/w[n] * (SA[ii+1] - SA[ii]) * 
                          (alpha_plus) + total[ii]);
                // Lewis and Miller Corrector
                if(ii == 0){
                    // center[n] = psi;
            ...
            for(int ii=0; ii < LENGTH; ii++){
                ...
                //         (alpha_plus) + total[ii]);
                psi = (mu[n] * (SA[ii+1] + SA[ii]) * psi_ihalf + 1/w[n] * (SA[ii+1] - SA[ii]) 
                        * (alpha_plus + alpha_minus) * psi_nhalf[ii] +  source[ii]) / 
                        (2 * mu[n] * SA[ii+1] + 2/w[n] * (SA[ii+1] - SA[ii]) * 
                        (alpha_plus) + total[ii]);
                // Lewis and Miller Corrector
                if(ii == 0){
                    // center[n] = psi;
