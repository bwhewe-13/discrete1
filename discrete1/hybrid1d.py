...
def auto_bdf1(flux_last, xs_total_u, xs_scatter_u, xs_fission_u, velocity_u, \
        external_u, boundary_u, medium_map, delta_x, angle_xu, angle_wu, \
        bc_x, angles_c, groups_c, energy_grid, steps, dt, geometry=1):
    ...
    return backward_euler(flux_last, xs_total_u, xs_total_c, xs_scatter_u, \
                xs_scatter_c, xs_fission_u, xs_fission_c, velocity_u, \
                velocity_c, external_u, boundary_u, medium_map, delta_x, \
                angle_xu, angle_xc, angle_wu, angle_wc, bc_x, fine_idx, \
                coarse_idx, factor, steps, dt, geometry)


def backward_euler(flux_last, xs_total_u, xs_total_c, xs_scatter_u, xs_scatter_c, \
        xs_fission_u, xs_fission_c, velocity_u, velocity_c, external_u, \
        boundary_u, medium_map, delta_x, angle_xu, angle_xc, angle_wu, \
        angle_wc, bc_x, fine_idx, coarse_idx, factor, steps, dt, geometry=1):
    ...
    for step in tqdm(range(steps), desc="BDF1*", ascii=True):
        ...
        # Run hybrid method
        _hybrid_method(flux_u, flux_c, xs_total_u, xs_total_c, xs_matrix_u, \
                xs_matrix_c, q_star, source_c, boundary_u[bb], boundary_c, \
                medium_map, delta_x, angle_xu, angle_xc, angle_wu, \
                angle_wc, bc_x, fine_idx, coarse_idx, factor, geometry)

        # Solve for angular flux
        flux_last = mg.known_source_angular(xs_total_u, q_star, boundary_u[bb], \
                                            medium_map, delta_x, angle_xu, \
                                            angle_wu, bc_x, geometry, edges=0)
...
def auto_bdf2(flux_last_1, xs_total_u, xs_scatter_u, xs_fission_u, velocity_u, \
        external_u, boundary_u, medium_map, delta_x, angle_xu, angle_wu, \
        bc_x, angles_c, groups_c, energy_grid, steps, dt, geometry=1):
    ...
    return bdf2(flux_last_1, xs_total_u, xs_total_c, xs_scatter_u, \
                xs_scatter_c, xs_fission_u, xs_fission_c, velocity_u, \
                velocity_c, external_u, boundary_u, medium_map, delta_x, \
                angle_xu, angle_xc, angle_wu, angle_wc, bc_x, fine_idx, \
                coarse_idx, factor, steps, dt, geometry)
...
def bdf2(flux_last_1, xs_total_u, xs_total_c, xs_scatter_u, xs_scatter_c, \
        xs_fission_u, xs_fission_c, velocity_u, velocity_c, external_u, \
        boundary_u, medium_map, delta_x, angle_xu, angle_xc, angle_wu, \
        angle_wc, bc_x, fine_idx, coarse_idx, factor, steps, dt, geometry=1):
    ...
    for step in tqdm(range(steps), desc="BDF2*", ascii=True):
        ...
        if step == 0:
            ...
            q_star = external_u[qq] + 2 / (velocity_u * dt) * flux_last_1 \
                                - 1 / (2 * velocity_u * dt) * flux_last_2

        # Run hybrid method
        _hybrid_method(flux_u, flux_c, xs_total_vu, xs_total_vc, xs_matrix_u, \
                xs_matrix_c, q_star, source_c, boundary_u[bb], boundary_c, \
                medium_map, delta_x, angle_xu, angle_xc, angle_wu, \
                angle_wc, bc_x, fine_idx, coarse_idx, factor, geometry)
        ...
        flux_last_1 = mg.known_source_angular(xs_total_vu, q_star, boundary_u[bb], \
                                            medium_map, delta_x, angle_xu, \
                                            angle_wu, bc_x, geometry, edges=0)
...
def _hybrid_method(flux_u, flux_c, xs_total_u, xs_total_c, xs_scatter_u, \
        xs_scatter_c, q_star, source_c, boundary_u, boundary_c, medium_map, \
        delta_x, angle_xu, angle_xc, angle_wu, angle_wc, bc_x, fine_idx, \
        coarse_idx, factor, geometry):
    
    # Step 1: Solve Uncollided Equation known_source (I x N x G) -> (I x G)
    flux_u = mg.known_source_scalar(xs_total_u, q_star, boundary_u, \
                                medium_map, delta_x, angle_xu, angle_wu, \
                                bc_x, geometry, edges=0)
    
    # Step 2: Compute collided source (I x G')
    tools._hybrid_source_collided(flux_u, xs_scatter_u, source_c, \
                                medium_map, coarse_idx)

    # Step 3: Solve Collided Equation (I x G')
    flux_c = mg.source_iteration(flux_c, xs_total_c, xs_scatter_c, \
                                source_c, boundary_c, medium_map, delta_x, \
                                angle_xc, angle_wc, bc_x, geometry)

    # Step 4: Create a new source and solve for angular flux
    tools._hybrid_source_total(flux_u, flux_c, xs_scatter_u, q_star, \
                                medium_map, coarse_idx, factor)