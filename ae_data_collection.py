...
import numpy as np
import os, argparse
# from .. import discrete1.theTruth as truth
# from discrete1 import theProcess as pro
import sys
print(sys.path)
...
for ii in range(len(enrich)):
    ...
    if usr_input.source is None:
        problem = truth.eigen_collect(*h.problem.variables(enrich[ii],ptype=usr_input.problem,symm=True),track=usr_input.track)
        ...
        problem = truth.source(*h.problem.variables(enrich[ii],ptype=usr_input.problem,symm=True),track=usr_input.track)
