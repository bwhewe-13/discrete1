...
class eigen_auto_djinn:
    ...
    def one_group(self,total,scatter,djinn_1g,external,guess,tol=1e-08,MAX_ITS=100):
        ...
        for n in range(self.N):
            ...
            if (self.multAE in ['scatter','both']):
                ...
                temp_scat = (djinn_1g).astype('float64') # How it is done in DJINN
            else:
                temp_scat = (scatter * phi_old).astype('float64')
            # Set Pointers for C function
            phi_ptr = ctypes.c_void_p(phi.ctypes.data)
    ...
    def multi_group(self,total,scatter,nuChiFission,guess,tol=1e-08,MAX_ITS=5000):
        """ Arguments:
        ...
        while not (converged):
            ...
            if self.multAE in ['scatter','both']:
                ...
                for g in range(self.G):
                    phi[:,g] = eigen_auto_djinn.one_group(self,total[:,g],scatter[:,g,g],smult[:,g],nuChiFission[:,g],phi_old[:,g])
                ...
                for g in range(self.G):
                    ...
                    phi[:,g] = eigen_auto_djinn.one_group(self,total[:,g],scatter[:,g,g],None,q_tilde,phi_old[:,g])
    ...
    def construct_scatter(self,flux):
        ...
        # Take only the flux using autoencoder (I x G) --> (I' x G)
        short_phi = sn.cat(flux,self.splits['scatter_djinn'])
        # Send through the Phi encoder (I' x G) --> (I' x G')
        reduced_flux = eigen_auto_djinn.scale_encoder(self,short_phi)
    ...
    def scale_encoder(self,flux):
        ...
        return encoded_flux # return reduced flux (I' x G')

    def create_djinn_scatter(self,flux,full_flux):
        """ flux is size (I' x G') 
        full_flux is (I' x G) """
        import numpy as np
        from discrete1.util import sn
        if np.sum(flux) == 0:
            ...
            # return np.zeros((sn.cat(flux,self.splits['scatter_djinn']).shape)) # Not needed?
        # Check for labeling
        if self.label:
            flux = np.hstack((sn.cat(self.enrich,self.splits['scatter_djinn'])[:,None],flux))
        # Predict DJINN model
        djinn_scatter_ns = self.dj_scatter.predict(flux)
        # Scale and recombine
        djinn_scatter = eigen_auto_djinn.scale_scatter(self,full_flux,djinn_scatter_ns)
        return djinn_scatter # (I' x G')

    def scale_scatter(self,full_phi,djinn_ns):
        """ full_phi is used for scaling purposes """
        import numpy as np
        from discrete1.util import sn
        # Calculate phi for DJINN part
        # short_phi = sn.cat(full_phi,self.splits['scatter_djinn'])
        # Calculate scaling factor
        scale = np.sum(full_phi*np.sum(sn.cat(self.scatter,self.splits['scatter_djinn']),axis=1),axis=1)/np.sum(djinn_ns,axis=1)
        # All of the sigma*phi terms not calculated by DJINN
        # regular = np.einsum('ijk,ik->ij',sn.cat(self.scatter,self.splits['scatter_keep']),sn.cat(full_phi,self.splits['scatter_keep']))
        # Scale DJINN
    ...
    def create_djinn_fission(self,flux):
        ...
        # Scale and recombine
        djinn_fission = eigen_auto_djinn.scale_fission(self,flux,djinn_fission_ns)
