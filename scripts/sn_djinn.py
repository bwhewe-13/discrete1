...
# if using only scatter
    ...
    save_folder = 'mydata/djinn_{}{}/scatter'.format(source_add,usr_input.smodel[0]); save_file = usr_input.smodel[1]
...
for ii in range(len(enrich)):
    ...
    if usr_input.source is None:
        problem = eigen_djinn(*s.problem1.variables(enrich[ii],problem=usr_input.problem),enrich=enrichment,splits=splits,track=usr_input.track,label=labeled)
