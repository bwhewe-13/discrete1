"""Anisotropic slab with in-sweep Legendre flux-moment accumulation."""
...
def discrete_ordinates(
    ...
    """Inner S_N sweep accumulating Legendre flux moments in-place."""
    ...
    # Precompute Legendre polynomials P[l, n] once
    P = np.array(
        [
            [legendre_polynomial(ll, angle_x[nn]) for nn in range(angles)]
            for ll in range(n_moments)
    ...
    # flux_moments[:,l] = sum_n w_n * P_l(mu_n) * psi(x, mu_n)
    # accumulated in-sweep, no psi storage needed
    ...
    while not converged:
        ...
        # Build anisotropic source from moments of previous inner iteration
        # S(x, mu_n) = sum_l (2l+1) * xs_l(x) * P_l(mu_n) * phi_l(x)
        weights = np.arange(n_moments) * 2 + 1
        aniso_source = np.einsum(
            "xl,xl,ln->xn",
            xs_scatter_cell * weights[np.newaxis, :],
            flux_moments,
        ...
        # Reset moments accumulator for this sweep
        flux_moments *= 0.0
        ...
        for nn in range(angles):
            ...
            if angle_x[nn] > 0.0:
                ...
                edge1 = slab_forward(
                    ...
                    angle_w[nn],
                    edges,
                    flux_moments,  # accumulated in-place
                    P[:, nn],  # P_l(mu_n) for all l
                )
...
def slab_forward(
    ...
    """Forward slab sweep accumulating flux moments in-place.

    Additional parameters
    ---------------------
    flux_moments : numpy.ndarray, shape (cells_x, L+1)
        Accumulated in-place: flux_moments[ii, l] += w_n * P_l(mu_n) * psi_ii
    P_n : numpy.ndarray, shape (L+1,)
        Legendre polynomials evaluated at mu_n, i.e. P[:, nn].
    """
    cells_x = medium_map.shape[0]
    edge2 = 0.0
    ...
    for ii in range(cells_x):
        ...
        flux_moments[ii] += angle_w * P_n * psi_ii
        if edges == 1:
...
def slab_backward(
    ...
    """Backward slab sweep accumulating flux moments in-place."""
    cells_x = medium_map.shape[0]
    edge2 = 0.0
