"""Anisotropic slab with vectorized Legendre moments (Sood PUa-1-2-SL)."""
...
def discrete_ordinates(
    ...
    from the angular flux moments accumulated during each sweep, so all
    Legendre moments are updated consistently each inner iteration without
    storing the full angular flux array.
    ...
    # Accumulated in-sweep without storing the full angular flux array.
    # Initialised to zero so the first iteration's aniso_source is zero,
    # which is the correct seed (scatter source from a zero angular flux).
        ...
        for nn in range(angles):
            ...
            if angle_x[nn] > 0.0:
                ...
                edge1 = slab_forward(
                    ...
                    edges,
                    flux_moments,
                    wP[:, nn],  # w_n * P_l(mu_n) for all l, shape (L+1,)
                )
...
def slab_forward(
    ...
    flux and accumulating angular flux moments in-place. No angular flux
    array is stored; moments are accumulated cell-by-cell using the local
    cell-centre value psi_ii = 0.5 * (edge1 + edge2).
    ...
    flux_moments : numpy.ndarray, shape (cells_x, L+1)
        Moment accumulator updated in-place:
        flux_moments[ii] += wP_n * psi_ii.
    wP_n : numpy.ndarray, shape (L+1,)
        Precomputed w_n * P_l(mu_n) for all l for this ordinate.
    ...
    for ii in range(cells_x):
        ...
        psi_ii = 0.5 * (edge1 + edge2)
        # Accumulate flux moments without storing psi:
        # flux_moments[ii, l] += w_n * P_l(mu_n) * psi_ii
        flux_moments[ii] += wP_n * psi_ii
        if edges == 1:
            flux[ii + 1] += angle_w * edge2
        else:
            flux[ii] += angle_w * psi_ii
        edge1 = edge2
